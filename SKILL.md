...
2. **Validate Credentials**
   ...
   **Troubleshooting validation**:
   ...
   - Use `--no-cache` flag to force fresh validation: `node scripts/validate-api-key.js --no-cache`

## Current API Permissions

<!-- PERMISSIONS_START -->
Run `node scripts/validate-api-key.js` to populate this section with your current permissions.
<!-- PERMISSIONS_END -->
...
```bash
# Using the skill
node scripts/workers.js deploy worker-name ./worker-script.js
```
...
Claude: [Deploys worker using node scripts/workers.js deploy api-handler ./worker.js]
...
node scripts/kv-storage.js write <namespace-id> "session:user123" '{"userId":"123","token":"abc"}'

# Read value
node scripts/kv-storage.js read <namespace-id> "session:user123"
...
# Delete a key
node scripts/kv-storage.js delete <namespace-id> "session:user123"
```
...
# Upload file
node scripts/r2-storage.js upload media-assets ./images/logo.png logo.png

# List objects
node scripts/r2-storage.js list-objects media-assets

# Download object
node scripts/r2-storage.js download media-assets logo.png ./downloaded-logo.png
```
...
node scripts/pages.js set-env my-app DEBUG true --env preview
...
2. **Create R2 bucket for media**
   ```bash
   node scripts/r2-storage.js create-bucket app-media
   ```

3. **Deploy worker with bindings**
   ```bash
   node scripts/workers.js deploy app-worker ./worker.js --kv-binding app-cache --r2-binding app-media
   ```

4. **Configure route**
   ```bash
   node scripts/dns-routes.js create-route example.com "example.com/*" app-worker
   ```
...
```bash
# Update worker code
node scripts/workers.js update worker-name ./new-worker-script.js

# Get worker details
node scripts/workers.js get worker-name
...
node scripts/kv-storage.js bulk-write namespace-name ./data.json

# Delete multiple keys
node scripts/kv-storage.js bulk-delete namespace-name key1 key2 key3
```
...
- Validate credentials after token updates: `node scripts/validate-api-key.js`
...
- **validate-api-key.js**: Validate API credentials and display permissions
- **workers.js**: Deploy, update, and manage Workers
- **kv-storage.js**: Create and manage KV namespaces and key-value pairs
- **r2-storage.js**: Create and manage R2 buckets and objects
- **pages.js**: Deploy and configure Cloudflare Pages projects
- **dns-routes.js**: Configure DNS records and worker routes
...
View Workers and Pages logs for debugging and monitoring:
...
Manage your Cloudflare zones and purge CDN cache:
...
# Purge by hostname
node scripts/zones.js purge-cache example.com --hosts api.example.com
```

### D1 Database Management

Create and manage D1 SQL databases:
...
node scripts/d1-database.js query <database-id> "SELECT * FROM users LIMIT 10"

# Execute SQL statement (INSERT, UPDATE, CREATE, etc.)
node scripts/d1-database.js execute <database-id> "CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT)"
node scripts/d1-database.js execute <database-id> "INSERT INTO users (name) VALUES ('John')"
...
**Solutions**:
1. List all namespaces: `node scripts/kv-storage.js list-namespaces`
...
3. Use existing bucket: `node scripts/r2-storage.js list-buckets`
...
**Solutions**:
1. Check deployment status: `node scripts/pages.js list-deployments project-name`
...
**Solutions**:
1. Verify zone exists: `node scripts/dns-routes.js list-zones`
