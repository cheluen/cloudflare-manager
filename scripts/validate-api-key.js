...
const CACHE_FILE = join(homedir(), '.claude/skills/cloudflare-manager/.permissions-cache.json');
...
async function validateApiKey() {
  try {
    ...
    const accountsResponse = await makeApiRequest('/accounts');
    ...
    try {
      const permissionsResponse = await makeApiRequest('/user/tokens/permission_groups');
      ...
      return {
        valid: true,
        permissions: [{ resource: 'Token verified', scope: 'Active' }],
        accountInfo: accountsResponse.result?.[0],
      };
    }
  } catch (error) {
 ...
 */
function updateSkillMd(permissions) {
  const skillMdPath = join(homedir(), '.claude/skills/cloudflare-manager/SKILL.md');
 ...
 */
function cacheValidation(result) {
  try {
    const cacheData = {
      timestamp: Date.now(),
      result,
